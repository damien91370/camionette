...
function spawnObjects(){
    ...
    if(Math.random()<0.7){
        let x=LANES[Math.floor(Math.random()*3)];
        obstacles.push({img:Math.random()<0.5?CAR_IMG:TRASH_IMG,x:x,y:-60,w:60,h:90});
    }
}

// --------- COLLISIONS ---------
function isColliding(a,b){
    return Math.abs(a.x-b.x)<30 && Math.abs(a.y-b.y)<60;
}
...
function loop(time){
    ...
    if(!gameOver){
        ...
        children.forEach((c,i)=>{if(isColliding(playerRect,c)){score++;children.splice(i,1);}});
        police.forEach((p,i)=>{if(isColliding(playerRect,p)){gameOver=true;}});
        obstacles.forEach((o,i)=>{if(isColliding(playerRect,o)){gameOver=true;}});
