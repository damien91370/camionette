...
<audio id="bgMusic" src="https://damien91370.github.io/camionnette/bossfight-Vextron.mp3" loop autoplay></audio>
...
function spawn(){
    for(let i=0;i<2;i++){
        children.push({x:randLane(),y:-40,w:40,h:50});
    }
    if(Math.random()<0.8) police.push({x:randLane(),y:-40,w:40,h:50});
    if(Math.random()<0.7) obstacles.push({x:randLane(),y:-60,w:60,h:80});
}
...
function drawBackground(offset){
    ...
    // espace entre lignes augmenté x1.65
...
//////////////////////////////////////////////////
function restart(){
    children=[]; police=[]; obstacles=[];
    score=0; playerLane=1; player.x=LANES[playerLane]; gameOver=false;
}
...
function loop(time){
    ...
    if(!gameOver){
        ...
        children.forEach((c,i)=>{
            c.y += speed;
            if(player.x-c.w/2 < c.x+c.w/2 && player.x+c.w/2 > c.x-c.w/2 &&
               player.y-c.h/2 < c.y+c.h/2 && player.y+c.h/2 > c.y-c.h/2){
                score++; children.splice(i,1);
            }
        });

        police.forEach((p,i)=>{
            p.y += speed;
            if(player.x-p.w/2 < p.x+p.w/2 && player.x+p.w/2 > p.x-p.w/2 &&
               player.y-p.h/2 < p.y+p.h/2 && player.y+p.h/2 > p.y-p.h/2){
                gameOver=true;
            }
        });
        ...
        obstacles.forEach((o,i)=>{
            o.y += speed;
            if(player.x-o.w/2 < o.x+o.w/2 && player.x+o.w/2 > o.x-o.w/2 &&
               player.y-o.h/2 < o.y+o.h/2 && player.y+o.h/2 > o.y-o.h/2){
                gameOver=true;
            }
        });
