...
function makeChild(){
    let s = document.createElement("canvas"); s.width=40; s.height=55;
    let c = s.getContext("2d");
...
function makeCar(){
    ...
    c.fillStyle="#ffff78"; c.beginPath(); c.arc(10,80,5,0,Math.PI*2); c.fill();
    c.beginPath(); c.arc(50,80,5,0,Math.PI*2); c.fill();
    return s;
}

function makeTrash(){
    let s = document.createElement("canvas"); s.width=50; s.height=70;
    let c = s.getContext("2d");
    c.fillStyle="#46be46"; c.fillRect(5,10,40,55);
    c.fillStyle="#329632"; c.fillRect(0,0,50,15);
    for(let y=20;y<60;y+=10){ c.strokeStyle="#28822a"; c.beginPath(); c.moveTo(8,y); c.lineTo(42,y); c.stroke(); }
    return s;
}
